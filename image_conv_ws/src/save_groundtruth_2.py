...
def get_tf_mat(translation, rotation):
    """Convert translation and rotation into a homogeneous transformation matrix."""
    T = np.eye(4)
    T[:3, :3] = rotation
    T[:3, 3] = translation.squeeze()  # Assuming translation is a column vector and needs to be squeezed
    return T
...
if __name__ == '__main__':
    ...
    cam_to_ee_data = read_cam_to_ee_transform('/fusionlab_files/data/bag13/cam_to_link7_quat.yaml')
    ...
    cam2_to_cam1_matrix = get_tf_mat(t21, rot21)

    cam2_to_ee_matrix = np.dot(cam_to_ee_matrix, cam2_to_cam1_matrix)
    ...
    while not rospy.is_shutdown():
        ...
        if link7_to_ground_transform is not None and timestamp is not None:
            ...
            # Concatenate the transformations to compute the transformation from the ground to the camera
            cam_to_ground_matrix = concatenate_matrices(link7_to_ground_matrix, cam2_to_ee_matrix)
            # Convert the resulting transformation matrix back to translation and rotation components
            ground_to_cam_transform = matrix_to_transform(cam_to_ground_matrix)
            # Write the transformation data to the groundtruth.txt file, including the timestamp
            # write_ground_truth(timestamp, ground_to_cam_transform, '/bagfiles/groundtruth.txt')
            # write_ground_truth(timestamp, ground_to_cam_transform, '/fusionlab_files/data/bag1/groundtruth.txt')
            write_ground_truth(timestamp, ground_to_cam_transform, '/fusionlab_files/data/bag13/groundtruth_new2.txt')
        rate.sleep()